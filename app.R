...
# df[,-c(1,2)] takes out columns 1 and 2 from df
...
# this contains the data with imputed missing values

# Adding country names to the rows
imputated_data$CountryName <-  df[,2] 
...
imputated_data <- imputated_data[,!grepl("CountryName",names(imputated_data))]
#dropping CountryName column as PCA cannot be applied to non numeric column
...
if (!require(lava)) install.packages("lava")
...
# Visualising the dissimalirty matrix. Visually shows distinct clusters. 
fviz_dist(dist(scaled_data), show_labels = FALSE)+ labs(title = "Euclidean distance")

#plotting just 20 countires :) 
randomplot <- sample(1:183, 20)  # choosing random 20 countries from 183 countries
mydata.sample<- scaled_data[randomplot,] # getting the data for those 20 countries
...
# Cluster visualisation dim1 vs dim2 gotten from pca
...
# rsconnect::setAccountInfo(name='dhruva1',
#                          token='23BAEB10154E18A6B1A3A074A7F7CFA5',
#                          secret='<>')
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      sliderInput("obs", "Number of countries:", min = 10, max = 183, value = 100)
    ),
    ...
    mainPanel(
      ...
      tabsetPanel(type = "tabs",
                  tabPanel("Cluster Plot on PCA dimension 1 vs dimension 2", plotOutput("plot1")),
                  tabPanel("World Map Clustered", plotOutput("plot2")),
                  tabPanel("PCA contributions", plotOutput("plot3"))
      )
